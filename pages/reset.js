...
const Forgot = ({toastShow}) => {
  ...
  console.log(router.query.forgot_token)
  ...
  useEffect(() => {
	...
	setForgotToken(router.query.forgot_token)
  }, [router])
  ...
  const handleSubmit = async(e) =>{
    e.preventDefault();
    const data = {forgotToken,password,cpassword};
        ...
        <div>
          <img className="mx-auto h-12 w-auto" src="/logo.webp" alt="Workflow"/>
            <p className="mt-2 text-center text-sm text-gray-600">
              Or
              <Link href={'/forgot'}><a href="#" className="font-medium text-pink-600 hover:text-pink-500"> Forgot Password </a></Link>
            </p>
        </div>
